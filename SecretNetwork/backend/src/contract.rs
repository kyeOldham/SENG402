...
// fn query_values<S: Storage, A: Api, Q: Querier>(deps: &Extern<S, A, Q>) -> StdResult<CountResponse> {
//   // let stored_state = get_config(&deps.storage)?;
//   let state = get_values(&deps.storage)?;

//   Ok(ValueResponse { Values: state.value.to_string()})
// }
